/*
** inetd service for spitting out null content. Integrate it with DNS to provide 
** advert blocking on a LAN or local host.
**    
** build:   make
** install: make install
** run:     Add the following to /etc/inetd.conf and restart inetd.
**
...
void send_headers(char* mime_type, int length) {  
    printf("%s %d %s\r\n", "HTTP/1.0", 200, "OK");
    printf("Server: %s\r\n", "nullserv");
    if (mime_type)
...
int main(int argc, char * argv[]) {
    ...
    if (querystring) querystring += sizeof(char);
	...
	// If debug was requested, then do it.
    if ( (querystring) && (strcasecmp(querystring, "debug") == 0) ) {
		printf("Protocol : %s %s\r\n", protocol, method);
		printf("Path     : %s\r\n", path);               
		printf("File     : %s\r\n", file);                    
		printf("Ext      : %s\r\n", ext);               
